...
if __name__ == "__main__":
    ...
    def benchmark(func, size, *args, n_runs=3):
        times = []
        ...
        print(f"Grid {size}x{size} -> Median: {median_t:.4f}s (min={min(times):.4f}, max={max(times):.4f})")
    ...
    for size in grid_sizes:
        t, M = benchmark(mandelbrot_set, size, -2, 1, -1.5, 1.5, size, size, 100, n_runs=3)